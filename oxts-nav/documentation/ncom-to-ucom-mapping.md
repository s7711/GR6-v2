...
| GadReject | GADLatestStatus | BNS_GAD | confirmed (2026-07-30) | NCOM's own manual (Table 83, Channel 95) documents this byte as "Number of consecutive updates rejected for this stream Id" — a rejection *count*, not a generic status code. `ncomrx.py`'s `decodeStatus95` previously stored it as `self.status['GADLatestStatus']`, borrowing UCOM's field name for an NCOM byte with a different, more specific documented meaning — renamed to `GADRejectedCount` to match what it actually is, and given the missing `0xFF`-is-invalid check every other field here already has. Found investigating why aruco's GAD position aiding never went active; separately, **Ben has confirmed with OXTS that this whole channel (95) is currently broken firmware-side on our xNAV650** (a stray pointer, not real data) — reported 2026-07-30, don't trust its live values until that's fixed. There's also GADPosStatus/GADVelStatus/GADAttStatus/GADAngRateStatus (per aiding-type reject status), which may be a richer replacement once UCOM is trustworthy. |
