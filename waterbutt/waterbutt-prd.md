...
duration, independent of `navigate`/`missions` (which don't yet do
anything with the water butt — see top-prd.md's future "mission" idea
...
- Any automatic "drive to the water butt, turn, wait, water" mission
  logic — that's the future `missions` service's job (see top-prd.md
